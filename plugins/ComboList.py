...
class ComboList(QtGui.QMainWindow):
    ...
    def __init__(self,name=None,items=None,colors=None, parent=None):
        ...
        if parent is not None:
            self.parent.sigSettingGUI.connect(self.setupParentMenu)

    def setupParentMenu(self):
        self.comboPlotMenu = self.parent.menuBar.addMenu('Combo plot')
        self.activateAction = QtGui.QAction('Show', self, shortcut='Ctrl+Shift+C')
        self.addSceneAction = QtGui.QAction('Add scene',self,shortcut='Ctrl+C')
        self.comboPlotMenu.addAction(self.activateAction)
        self.comboPlotMenu.addAction(self.addSceneAction)
        self.activateAction.triggered.connect(self.show)
        self.addSceneAction.triggered.connect(self.addScene)
        self.comboPlotMenu.setDisabled(True)
        self.parent.sigConnectParameters.connect(self.activateParentMenu)

    def activateParentMenu(self):
        self.comboPlotMenu.setDisabled(False)

    def addScene(self):
        rootname = self.parent.currentDataSetName
        for item in self.parent.plt.listDataItems():
